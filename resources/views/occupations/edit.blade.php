<x-modal-layout>
    <x-auth-card>
        ...
        @can('create-occupation', $occupation->date)
            ...
            @elseif ($occupation->user_id == $user->id)
            ...
            @endif
        @else
            <h2 class="text-xl font-bold leading-none text-left lg:text-3xl">Bereits alle Plätze reserviert</h2>
            <p class="mt-4">Leider haben Sie für diese Woche bereits alle Plätze reserviert.</p>
            <div class="flex items-center justify-end mt-4">
                <x-button-link class="ml-4" :href="route('occupations.create', $location)">
                    {{ __('Zurück zur Übersicht') }}
                </x-button-link >
